...
appendages_data <- data_1 %>%
  group_by(appendage) %>%
  filter(appendage == "smasher") #strings need quotes 

lm_1 <- lm(cor.telson~cor.abdomen, data_1)
summary(lm_1)[[9]]#number assocaited with chunk 


ggplot(data_1, aes(x=cor.abdomen,y=cor.telson))+
  geom_point(shape=1)+
  geom_smooth(method=lm)
#look at claudia repo 
...
```{r}
rmarkdown::render(input = "Mantis_Shrimp_Mini_Project.Rmd", output_format = "html_document", output_dir="../output")
```
